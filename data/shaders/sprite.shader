...
void main(){
	...
	gl_Position =  MVP * vec4(0.0,0.0,0.0,1.0) + vec4(vertexPosition_modelspace + dlt,0.0);
	UV = vertexUV;	
}
...
void main(){
	lowp vec4 MaterialDiffuseColor = texture2D( DiffuseTextureSampler, UV ).rgba;
	if(MaterialDiffuseColor.a < 0.5)
	{
		discard;
	}
	color = MaterialDiffuseColor.rgb;
}
