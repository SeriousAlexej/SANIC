...
void main(){
	...
	lowp vec3 toViewer = normalize(TBN * (cameraPos - Position_worldspace));
	toViewer.y *= -1.0;
	...
	lowp float h = texture(HeightTextureSampler, parallaxUV).r;

	while(h < height)
	{
		height -= step;
		parallaxUV += dtex;
		h = texture(HeightTextureSampler, parallaxUV).r;
	}
	...
	for(int i=0; i<5; i++)
	{
		...
		if(texture(HeightTextureSampler, mid).r < height)
		{
			mid += delta;
		} else {
			mid -= delta;
	...
	lowp vec4 MaterialDiffuseColora = texture(DiffuseTextureSampler, parallaxUV).rgba;
	if(MaterialDiffuseColora.a < 0.5)
	{
		discard;
	}
	lowp vec3 MaterialDiffuseColor = MaterialDiffuseColora.rgb;
	lowp vec3 n = TBN * (1.9921875 * texture(NormalTextureSampler, parallaxUV).xyz - 1.0);
