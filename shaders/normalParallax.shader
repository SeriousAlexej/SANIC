...
void main(){
	lowp vec3 toViewer = normalize(cameraPosition - Position_worldspace);
	lowp vec2 tvTBN = (toViewer * TBN).xy;
	tvTBN.x *= -1.0;
	lowp float parallaxBias = 0.5 * parallaxScale * (parallaxOffset - 1.0);
	lowp vec2 parallaxUV = UV.xy + tvTBN * (texture(HeightTextureSampler, UV.xy).r * parallaxScale + parallaxBias);
	...
	lowp vec3 n = TBN * (255.0/128.0 * texture(NormalTextureSampler, parallaxUV).xyz - 1.0);
